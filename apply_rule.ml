...
let apply_rule_with_exceptions request_as_json auto_reverse =
    ...
    if auto_reverse then List.map Proof.rec_apply_reversible_rule premises
    else premises

let apply_rule request_as_json auto_reverse =
    try let premises = apply_rule_with_exceptions request_as_json auto_reverse in
